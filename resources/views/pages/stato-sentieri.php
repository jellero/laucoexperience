...
try {
    $trailRows = $pdo->query(
        "SELECT p.id,p.titolo,p.slug,p.tipo,p.localita,s.stato,s.nota_pubblica,s.ultima_verifica_at "
        . "FROM percorsi p LEFT JOIN stato_sentieri s ON s.percorso_id=p.id AND s.pubblicato=1 "
        . "WHERE p.pubblicato=1 ORDER BY FIELD(COALESCE(s.stato,'in_verifica'),'non_percorribile','attenzione','in_verifica','verificato'),p.ordine,p.titolo"
    )->fetchAll() ?: [];
} catch (Throwable) {
    $trailRows = [];
}
$trailStatusKeys = [
    'verificato' => 'trail_status.verified', 'attenzione' => 'trail_status.attention',
    'non_percorribile' => 'trail_status.closed', 'in_verifica' => 'trail_status.pending',
];
?>
<!DOCTYPE html>
<html lang="it">
    ...
    <div id="page-content" class="header-static">
        ...
        <main id="page-wrap" class="content-section fullpage-wrap trail-status-page">
            <div class="container text">
                ...
                <p class="trail-status-intro"><?= e(site_text('trail_status.intro', null, 'Informazioni operative aggiornate dopo i controlli disponibili sul territorio.')) ?></p>
                <div class="trail-status-filters" role="group">
                    ...
                    <?php foreach ($trailStatusKeys as $status => $key): ?><button type="button" class="trail-status-filter" data-status="<?= e($status) ?>"><?= e(site_text($key, null, volontariato_status_label($status))) ?></button><?php endforeach; ?>
                </div>
                <?php if ($trailRows === []): ?>
                    <p class="trail-status-empty"><?= e(site_text('trail_status.empty', null, 'Lo stato dei percorsi è in fase di aggiornamento.')) ?></p>
                <?php else: ?>
                    <div class="trail-status-grid">
                        <?php foreach ($trailRows as $trail): $status=(string)($trail['stato'] ?: 'in_verifica'); ?>
                            <article class="trail-status-card" data-status="<?= e($status) ?>">
                                <div>
                                    <small><?= e(strtoupper((string)$trail['tipo'])) ?><?= !empty($trail['localita']) ? ' · '.e($trail['localita']) : '' ?></small>
                                    <h3><?= e($trail['titolo']) ?></h3>
                                    <?php if (!empty($trail['nota_pubblica'])): ?><p><?= nl2br(e($trail['nota_pubblica'])) ?></p><?php endif; ?>
                                    ...
                                    <a class="trail-status-link" href="/percorso?slug=<?= rawurlencode((string)$trail['slug']) ?>"><?= e(site_text('trail_status.open_route', null, 'Apri itinerario')) ?></a>
                                </div>
                                <span class="trail-status-badge <?= e($status) ?>"><?= e(site_text($trailStatusKeys[$status] ?? 'trail_status.pending', null, volontariato_status_label($status))) ?></span>
                            </article>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
