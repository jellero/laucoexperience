...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!admin_whatsapp_action_allowed(admin_role(), $action)) {
        admin_access_denied('Il ruolo Operatore WhatsApp può gestire soltanto gruppi, inviti e conversazioni WhatsApp.');
    }
    try {
...
<main class="wrap">
<section class="hero-admin"><h1><?=admin_can('admin.all')?'Volontariato e cura del territorio':'Gestione WhatsApp'?></h1><p><?=admin_can('admin.all')?'Un solo pannello per disponibilità, gruppi WhatsApp, conversazioni e attività.':'Gestisci gruppi, inviti e conversazioni WhatsApp.'?></p></section>
...
<?php $tabLabels=['overview'=>'Riepilogo','volontari'=>'Volontari','gruppi'=>'Gruppi','chat'=>'Chat WhatsApp','attivita'=>'Planning'];foreach($allowedViews as $key):$label=$tabLabels[$key];?><a class="<?=$view===$key?'active':''?>" href="?view=<?=e($key)?>"><?=e($label)?><?php if($key==='chat'&&$summary['non_letti']>0):?> (<?=$summary['non_letti']?>)<?php endif;?></a><?php endforeach;?>
</nav>

<?php if($view==='overview'):?>
<section class="vol-summary"><div class="vol-stat">Volontari<strong><?=$summary['volontari']?></strong><small><?=$summary['attivi']?> attivi</small></div><div class="vol-stat">Gruppi attivi<strong><?=$summary['gruppi']?></strong></div><div class="vol-stat">Chat da leggere<strong><?=$summary['non_letti']?></strong></div><div class="vol-stat">Attività aperte<strong><?=$summary['attivita']?></strong></div></section>
