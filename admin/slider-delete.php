...
function request_slider_id(): int
{
    return (int) ($_POST['id'] ?? $_GET['id'] ?? $_POST['slider_id'] ?? $_GET['slider_id'] ?? 0);
}
...
if (!$id) {
    header('Location: slider.php?msg=' . urlencode('Slide non valida.'));
    exit;
}

$uploadRoot = dirname(__DIR__);

function delete_slider_upload(?string $relativePath): void
{
    global $uploadRoot;
...
if (!$slide) {
    header('Location: slider.php?msg=' . urlencode('Slide non trovata.'));
    exit;
}

delete_slider_upload($slide['image_path'] ?? null);

$del = $pdo->prepare('DELETE FROM home_slider WHERE id = :id');
$del->execute(['id' => $id]);

header('Location: slider.php?msg=' . urlencode('Slide eliminata.'));
exit;
