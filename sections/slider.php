...
function slider_url(array $slide): string
{
    switch ($slide['link_type']) {
        case 'evento':
            return !empty($slide['evento_slug']) ? 'evento.php?slug=' . urlencode($slide['evento_slug']) : '#';

        case 'percorso':
            return !empty($slide['percorso_slug']) ? 'percorso.php?slug=' . urlencode($slide['percorso_slug']) : '#';
