...
(function($) {
    ...
    function sendContactForm(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.info !== 'error') {
                    contactForm.find('input[type=text], input[type=email], textarea, select').filter(':visible').val('');
            ...
            error: function(xhr) {
                var msg = 'Errore durante l’invio. Riprova tra poco.';
                ...
                message.hide().removeClass('success').removeClass('error').addClass('error').html(msg).fadeIn('slow').delay(5000).fadeOut('slow');
            },
            complete: function() {
    ...
    submitContact.off('click').on('click', sendContactForm);
    contactForm.off('submit').on('submit', sendContactForm);
})(jQuery);
