...
function eventi_archivio_date(?string $date): string
{
    if (!$date) {
        return '';
    }
...
function eventi_archivio_categories(?string $category): array
{
    if (!$category) {
        return ['Eventi'];
    }
...
$stmt = $pdo->prepare("
    ...
    ORDER BY data_evento DESC, ordine ASC, created_at DESC
");
                ...
                <ul class="slides">
                    <li style="background-image:url(<?= e($hero) ?>)">
                        <div class="container text text-center">
                            <h1 class="white margin-bottom-small">Archivio eventi</h1>
                            <p class="heading white">
                                Gli appuntamenti già trascorsi di Lauco Experience.
                            </p>
                        </div>
                ...
                <div class="row margin-leftright-null grey-background eventi-page-section">
                    <div class="container">
                        <div class="col-md-12 padding-leftright-null text padding-bottom-null text-center">
                            <h2 class="margin-bottom-null title line center">ARCHIVIO EVENTI</h2>
                            <p class="heading center grey margin-bottom-null">
                                Gli eventi conclusi, dal più recente a quelli meno recenti.
                            </p>
                        </div>
                        ...
                        <div class="col-md-12 eventi-grid">
                            <?php if (!$eventi): ?>
                                <div class="eventi-empty text-center">
                                    <p class="margin-bottom-null">L’archivio eventi è ancora vuoto.</p>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php foreach ($eventi as $evento): ?>
                                <?php
                                    ...
                                    $date = eventi_archivio_date($evento['data_evento'] ?? null);
                                    $categories = eventi_archivio_categories($evento['categoria'] ?? '');
                            ...
                            <div class="eventi-back-link">
                                <a href="/eventi" class="btn-alt small margin-null">Torna ai prossimi eventi</a>
                            </div>
                        </div>
                    </div>
                </div>
